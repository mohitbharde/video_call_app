...
//   }
//   const { peer, ws, shutdown, setShutdown } = socketContext;
...
const VideoCall = () => {
  ...
  const [video, setVideo] = useState<MediaStreamTrack | null>(null);
  const [audio, setAudio] = useState<MediaStreamTrack | null>(null);
  const [videoIcon, setVideoIcon] = useState(videoControl);
  const [audioIcon, setAudioIcon] = useState(audioControl);
  const navigate = useNavigate();
  ...
  const iconArray = [
    {
      ...
      onclick: () => {
        if (video) {
          video.enabled = !video.enabled;
          setVideoIcon(videoIcon === videoControl ? videoOff : videoControl);
        }
      },
    },
    {
      ...
      onclick: () => {
        if (audio) {
          audio.enabled = !audio.enabled;
          setAudioIcon(audioIcon === audioControl ? muted : audioControl);
        }
      },
    },
    {
        ...
        try {
          ...
          display.getVideoTracks()[0].onended = () => {
            if (videoSender && video) {
              ...
              if (videoRef.current) {
                const tracks: MediaStreamTrack[] = [video, audio].filter(
